...
def find_all_cols_idx(norm_cols: List[str], must_have: Set[str]) -> List[int]:
    idxs = []
    for i, c in enumerate(norm_cols):
        if must_have.issubset(tokens(c)):
            idxs.append(i)
    return idxs
...
def parse_trigger_codes(cell: Any) -> List[str]:
    """
    Trigger Variable cells in your sheet are codes like:
      "1,2,3,4" or "3" or "1-4"
    Sometimes could be "1;2" or with spaces.

    We ONLY extract codes (digits), expand ranges, dedupe.
    """
    # numeric cell
...
# Build minimal JSON
# -----------------------------
def build_minimal_json(df: pd.DataFrame, debug: bool = False) -> List[Dict[str, Any]]:
    ...
    def add_edge(p: str, c: str, codes: List[str], order: int) -> None:
        if p == "UNKNOWN" or c == "UNKNOWN":
            return
        # if trigger codes are empty, skip (or change to keep empty if you want)
        if not codes:
            return
        k = (p, c)
        if k not in edges:
...
def main():
    ...
    ap.add_argument("--debug", action="store_true", help="Print detected columns")
    args = ap.parse_args()

    df = pd.read_excel(args.input, sheet_name=args.sheet)
    out = build_minimal_json(df, debug=args.debug)
...
## python .\sohea_excel_to_json_2025.py --input "SOHEA hierarchical questions mapping 2023 to 2025.xlsx" --sheet "Final Question Mapping 2025" --out "SOHEA_Questions_mapping_v1.json" --debug