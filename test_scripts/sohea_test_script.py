...
import argparse
parser = argparse.ArgumentParser(description='Run SOHEA test script with specified arguments.')
...
# SOHEA_TEST_CASES = load_test_cases('./test_scripts/sohea_test_cases.xlsx')
SOHEA_TEST_CASES = load_test_cases_csv('./test_scripts/sohea_test_cases.csv') # you can switch to csv or excel test cases as needed
overall_start_time = datetime.now()

# exist_file_path = f'./test_scripts/test_reports/sohea_test_results_20260313_153906.xlsx'
...
if exist_file_path and os.path.exists(exist_file_path):
    ...
    file_path = f"./test_scripts/test_reports/sohea_test_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    summary = prepare_summary(no_of_iters)
print(f"Test results will be saved to: {file_path}")
if question_ids:
    SOHEA_TEST_CASES_ = [SOHEA_TEST_CASES[i] for i in range(len(SOHEA_TEST_CASES)) if i+1 in question_ids]
else:
    SOHEA_TEST_CASES_ = SOHEA_TEST_CASES # run all test cases
for test_case in SOHEA_TEST_CASES_:
    print(f"\nProcessing test case: {test_case}")
    try:
        question_number = SOHEA_TEST_CASES.index(test_case) + 1
...
print(f"Time taken for the entire test suite: {overall_end_time - overall_start_time} no.of test cases: {len(SOHEA_TEST_CASES_)} no. of iterations per test case: {no_of_iters}")

# how to run this script:

#  python -m test_scripts.sohea_test_script --no_of_iters 3 --exist_file_path ./test_scripts/test_reports/sohea_test_results_20260313_153906.xlsx --question_ids 1 3 5
# This will run test cases Q1, Q3, and Q5 with 3 iterations each, and it will save the results to the specified existing file path.


#  python -m test_scripts.sohea_test_script --no_of_iters 3
# This will run all test cases with 3 iterations each, and it will save the results to a new file with a timestamp in the name.